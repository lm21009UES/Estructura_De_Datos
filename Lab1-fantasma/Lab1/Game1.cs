    ...
    public class Game1 : Game
    {
        ...
        public Game1()
        {
           ...
           this._graphics.PreferredBackBufferHeight = 767;//alto
            ...
            //version--detalles juego
            puntaje = 0; // Resetea el puntaje 
            nivel = 1; // Resetea el nivel 
        ...
        void crearpuntos()
        {
            ...
            for (int i = 0; i < 29; i++)
            {
                for (int j = 0; j < 27; j++)
                {
                    counter++;
                    Rectangle tempRect;
                    if ((i == 0 && j==0) || (i ==0 && j==25) || (i==22 && j == 0) || (i == 25 && j == 25))
                    {
                        tempRect = new Rectangle(x + (j * 24), y + (i * 21), 16, 15);
                    }
                    else
                    {
        ...
        protected override void LoadContent()
        {
            ...
            //version detalle juego
            gameover = Content.Load<Texture2D>("gameover");
            puntajeTexto = Content.Load<SpriteFont>("texto");

            try
            {
                string ruta = Path.Combine("score.txt");
                StreamReader conjuntocaracteres = File.OpenText(ruta);

                maximopuntaje = int.Parse(conjuntocaracteres.ReadToEnd());
                conjuntocaracteres.Close();
            }
            catch(Exception e)
            {
                string nsg = e.ToString();
                maximopuntaje = 0;
            }
        ...
        protected override void Draw(GameTime gameTime)
        {
           ...
           //cerrar la puerta
            ...
            for (int i = 0; i < noFantasmas; i++)
            {
                ...
                if (fantasmaeditable)
                {
                    _spriteBatch.Draw(fantasmaboca, new Rectangle(fantasmaX[i] + 2, fantasmaY[i] + 16, fantasmaboca.Width, fantasmaboca.Height), Color.White);
                }
                if (tempContador == 4)
                    tempContador = -1;
            }
            ...
            if (finJuego)
            {
                _spriteBatch.Draw(gameover, new Rectangle(0, 0, 1030, 740), Color.White);
               // noFantasmas = totalfan;
            }

            string scoreString = puntaje.ToString("0\n0\n0\n0\n0");
            _spriteBatch.DrawString(puntajeTexto, scoreString, new Vector2(60.0f, 110.0f), Color.White);
            for (int i = 0; i < (vidas - 1); i++)
            {
                float tempTop = (float)spriteDerecha.Height * (float)i * 1.3f;
                _spriteBatch.Draw(spriteDerecha, new Rectangle(53, 570 + (int)tempTop, spriteDerecha.Width, spriteDerecha.Height), Color.White);
            }
            _spriteBatch.End();
                    ...
                    if (temprect.Intersects(pts[i]))
                    {
                        if (juegoCorriendo)
                        {
                            if ((pts[i] == new Rectangle(177 + (0 * 24) - 4, 42 + (0 * 21) - 4, 16, 15)) ||
                                ...
                                (pts[i] == new Rectangle(177 + (25 * 24) - 4, 42 + (25 * 21) - 4, 16, 15)))
                            {
                                puntaje += (nivel * 20);
                ...
                if (temprect.Intersects(new Rectangle(spriteX, spriteY, spriteArriba.Width, spriteAbajo.Height)))
                {
                    ...
                    else
                    {
                        if (!muriendo)
                        {
                            vidas--;
                            muriendo = true;
                            inicioJuego(puntaje, nivel, vidas, false);
                            muriendo = false;
                            if (vidas == 0)
                                finJuego = true;
                        }
